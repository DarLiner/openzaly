        ...
        try {
            ...
            if (userSignResult) {
                ...
                if (!loginResult) {// 更新失败，则重新保存数据
                    ...
                    List<UserDeviceBean> userDeviceList = userDeviceDao.getUserDeviceList(siteUserId);
                    System.out.println(userDeviceList);
                    if (userDeviceList.size() >= 4) {
                        int i = userDeviceDao.limirDeviceNum(siteUserId);
                        if (i == 0) {
                            logger.error("limitDeviceNum failed");
                        }
                    }
                }
