...
public class MigrateUtils {
	...
	public static void sqlite2Mysql(Properties prop) throws MigrateDatabaseException {
	...
	private static boolean doSiteGroupProfileTable() throws MigrateDatabaseException {
		...
		String toSql = "insert into values(?,?,?,?,?,?,?,?,?,?);";
	...
	private static boolean doMigrateWork(String fromSql, String toSql, final String rsMethodName)
		...
		try {
			...
			try {
				...
				while (rs.next()) {
					reflectRsMethod(rsMethodName, fromPst, rs);
				}
			...
			throw new MigrateDatabaseException("migrate databse sqlite to mysql error", e);
		} finally {
